...
<#import "components/config/branding.ftl" as branding>
...
>
  <#assign brand=branding.resolve()>

  <#assign cardHeader>
    <@logo.kw brand=brand>
      ${kcSanitize(msg("loginTitleHtml", (brand.name!realm.displayNameHtml!"")))?no_esc}
    </@logo.kw>
  ...
  <#assign cardContent>
    ...
    <main class="ac-main" id="ac-main-content">
      <#nested "form">
  ...
  <#assign cardFooter>
    ...
    <p class="ac-footnote" aria-label="${msg('acronimoBrandingInfo')}">
      ${brand.shortCopy}
    </p>
  </#assign>

  <html lang="${(locale.currentLanguageTag)!'en'}" data-theme="${brand.mode}">
    <head>
      <@document.kw brand=brand script=script />
    </head>
    <@body.kw>
      <a class="ac-skip-link" href="#ac-main-content">${msg("acronimoSkipToContent")}</a>
      <@container.kw>
